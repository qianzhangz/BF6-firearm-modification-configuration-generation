...
void Gun1(double hipWeight, double accuracyWeight, double controlWeight, double mobilityWeight) {
    ...
    for (int i = 0; i < 10; ++i) { // 输出每部分选中的配件信息
        cout << "第" << i + 1 << "部分，选用配件编号 " << selected[i]
            << "，配件名称：" << parts[i][selected[i]].name << endl;
    }
    cout << "腰射：" << hip << "（代表腰射能力）" << endl;         // 输出腰射参数
    cout << "精准：" << accuracy << "（代表精准能力）" << endl;   // 输出精准参数
    cout << "控制：" << control << "（代表控制能力）" << endl;    // 输出控制参数
    cout << "机动：" << mobility << "（代表机动能力）" << endl;   // 输出机动参数
    cout << "加权总分：" << weightedScore << "（代表综合评分）" << endl; // 输出加权总分
}

int main() {
    while (true) { // 循环，支持重新配置
        cout << "请选择枪械编号喵！（1-45）请看上方输出的表格喵！：" << endl; // 提示输入枪械编号
        int gunIdx; // 枪械编号变量
        cin >> gunIdx; // 读取用户输入的枪械编号
        double hipWeight, accuracyWeight, controlWeight, mobilityWeight; // 权重变量
        cout << "是否可以接受使用机械瞄具喵！"<<endl;
       ...
       // }
        ...
        while (true) {
            ...
            if (totalWeight > 100.0) { // 如果总和超过100
                cout << "参数权重总和不能超过100%，请重新输入喵！" << endl; // 提示重新输入
            }
            else {
                break; // 合法则跳出循环
            }
        ...
        case 1: Gun1(hipWeight, accuracyWeight, controlWeight, mobilityWeight); break; // 调用1号枪械函数
        default: cout << "枪械编号无效喵！" << endl; break; // 输入无效编号时提示
        }

        // 询问是否退出或重新配置
        cout << "是否开始退出程序喵？输入 y 退出，输入其他任意键重新配置喵！："; // 提示用户选择
        string choice; // 用户选择变量
        cin >> choice; // 读取用户输入
        if (choice == "y" || choice == "Y") break; // 输入y或Y则退出循环
    }
    cout << "要不要进作者b站主页给个关注喵！反正也不要钱的喵！可以的话请输入“y”的喵！（哼，不行的话就随便按个按键吧，可不能是关机键什么的哦！记得按回车哦！）";
    ...
    if (choiceup == "y" || choiceup == "Y") {
        ...
        cout << "谢谢使用喵！欢迎下次使用喵！" << endl;
    } else {
		cout << "谢谢使用喵！欢迎下次使用喵！" << endl; // 输出感谢信息
    }
    return 0; // 程序结束
}